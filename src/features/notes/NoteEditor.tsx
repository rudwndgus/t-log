...
import { useCallback, useEffect, useRef, useState, type ButtonHTMLAttributes, type Dispatch, type KeyboardEvent, type SetStateAction } from 'react'
...
export function NoteEditor({ pageId, blocks, onBlocksChange, onAction }: { pageId: string; blocks: NoteBlock[]; onBlocksChange: (blocks: NoteBlock[]) => void; onAction: (target: ActionTarget) => void }) {
  ...
  const chooseCommand = useCallback((command: NoteCommand, currentBlocks: NoteBlock[], index: number, commit: (next: NoteBlock[]) => void) => {
    ...
    const converted: NoteBlock = { ...current, type: command.type, content: '', checked: command.type === 'todo' ? false : undefined, children: command.type === 'toggle' ? [createNoteBlock(uid())] : undefined, collapsed: command.type === 'toggle' ? true : undefined }
    ...
    commit(currentBlocks.map((block, blockIndex) => blockIndex === index ? converted : block)); setSlash(null); focusBlock(converted.id)
  }, [focusBlock])
  return <div className="notion-editor"><EditorList blocks={blocks} level={0} onBlocksChange={onBlocksChange} onAction={onAction} activeId={activeId} setActiveId={setActiveId} slash={slash} setSlash={setSlash} registerInput={registerInput} focusBlock={focusBlock} chooseCommand={chooseCommand} /><button type="button" className="note-inline-add" onClick={() => { const block = createNoteBlock(uid(), 'paragraph', '/'); onBlocksChange([...blocks, block]); setSlash({ blockId: block.id, query: '', selected: 0 }); focusBlock(block.id) }}><Plus size={16} /> 블록 추가</button></div>
}

function EditorList({ blocks, level, onBlocksChange, onAction, activeId, setActiveId, slash, setSlash, registerInput, focusBlock, chooseCommand }: { blocks: NoteBlock[]; level: number; onBlocksChange: (blocks: NoteBlock[]) => void; onAction: (target: ActionTarget) => void; activeId: string | null; setActiveId: Dispatch<SetStateAction<string | null>>; slash: SlashState | null; setSlash: (state: SlashState | null) => void; registerInput: (id: string, node: HTMLTextAreaElement | null) => void; focusBlock: (id: string, caret?: number) => void; chooseCommand: (command: NoteCommand, blocks: NoteBlock[], index: number, commit: (next: NoteBlock[]) => void) => void }) {
  ...
  return <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}><SortableContext items={blocks.map((block) => block.id)} strategy={verticalListSortingStrategy}><div className={`editor-blocks ${level ? 'editor-blocks--nested' : ''}`}>{blocks.map((block, index) => <SortableBlock key={block.id} block={block} index={index} number={block.type === 'numbered' ? consecutiveNumber(blocks, index) : undefined} level={level} blocks={blocks} commit={commit} updateAt={updateAt} removeAt={removeAt} onAction={onAction} activeId={activeId} setActiveId={setActiveId} slash={slash} setSlash={setSlash} registerInput={registerInput} focusBlock={focusBlock} chooseCommand={chooseCommand} />)}</div></SortableContext></DndContext>
}

function SortableBlock({ block, index, number, level, blocks, commit, updateAt, removeAt, onAction, activeId, setActiveId, slash, setSlash, registerInput, focusBlock, chooseCommand }: { block: NoteBlock; index: number; number?: number; level: number; blocks: NoteBlock[]; commit: (blocks: NoteBlock[]) => void; updateAt: (index: number, changes: Partial<NoteBlock>) => void; removeAt: (index: number) => void; onAction: (target: ActionTarget) => void; activeId: string | null; setActiveId: Dispatch<SetStateAction<string | null>>; slash: SlashState | null; setSlash: (state: SlashState | null) => void; registerInput: (id: string, node: HTMLTextAreaElement | null) => void; focusBlock: (id: string, caret?: number) => void; chooseCommand: (command: NoteCommand, blocks: NoteBlock[], index: number, commit: (next: NoteBlock[]) => void) => void }) {
  ...
  const handleChange = (value: string, textarea: HTMLTextAreaElement) => {
    ...
    updateAt(index, { content: value }); const query = slashQuery({ ...block, content: value }); setSlash(query === null ? slash?.blockId === block.id ? null : slash : { blockId: block.id, query, selected: 0 })
