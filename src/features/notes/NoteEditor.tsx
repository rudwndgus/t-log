import { closestCenter, DndContext, KeyboardSensor, PointerSensor, TouchSensor, useDroppable, useSensor, useSensors, type DragEndEvent } from '@dnd-kit/core'
import { arrayMove, SortableContext, sortableKeyboardCoordinates, useSortable, verticalListSortingStrategy } from '@dnd-kit/sortable'
import { CSS } from '@dnd-kit/utilities'
import { Plus } from 'lucide-react'
import { Fragment, useCallback, useEffect, useRef, useState, type ButtonHTMLAttributes, type ChangeEvent, type Dispatch, type KeyboardEvent, type ReactNode, type SetStateAction } from 'react'
...
import { consecutiveNumber, continuationType, createNoteBlock, findNoteBlockLocation, isTextBlock, markdownBlockType, moveNoteBlock, normalizeNoteBlocks, replaceNoteBlock, slashQuery } from './noteEditorUtils'
...
interface DropTarget { parentId: string | null; index: number }
...
export function NoteEditor({ tripId, pageId, userId, blocks, onBlocksChange, onAction, onDeleteAttachment }: { tripId: string; pageId: string; userId: string; blocks: NoteBlock[]; onBlocksChange: (blocks: NoteBlock[]) => void; onAction: (target: NoteActionTarget) => void; onDeleteAttachment: (block: NoteBlock) => void }) {
  ...
  const [draggingId, setDraggingId] = useState<string | null>(null); const [dropTarget, setDropTarget] = useState<DropTarget | null>(null)
  ...
  const resolveDropTarget = (id: string | number): DropTarget | null => {
    const value = String(id)
    if (value.startsWith('drop:')) { const [, parent, rawIndex] = value.split(':'); return { parentId: parent === 'root' ? null : parent, index: Number(rawIndex) } }
    const location = findNoteBlockLocation(blocksRef.current, value); return location ? { parentId: location.parentId, index: location.index } : null
  }
  const finishDrag = ({ active, over }: DragEndEvent) => {
    const target = over ? resolveDropTarget(over.id) : dropTarget
    if (target) commitBlocks(moveNoteBlock(blocksRef.current, String(active.id), target.parentId, target.index))
    setDraggingId(null); setDropTarget(null)
  }
  return <div className="notion-editor"><DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={({ active }) => setDraggingId(String(active.id))} onDragOver={({ over }) => setDropTarget(over ? resolveDropTarget(over.id) : null)} onDragCancel={() => { setDraggingId(null); setDropTarget(null) }} onDragEnd={finishDrag}><EditorList blocks={blocks} parentId={null} level={0} onBlocksChange={commitBlocks} onAction={onAction} onRemoveBlock={removeBlock} renderMedia={media} activeId={activeId} setActiveId={setActiveId} slash={slash} setSlash={setSlash} registerInput={registerInput} focusBlock={focusBlock} chooseCommand={chooseCommand} dragging={Boolean(draggingId)} dropTarget={dropTarget} /></DndContext><button type="button" className="note-inline-add" onClick={() => { const block = createNoteBlock(uid(), 'paragraph', '/'); commitBlocks([...blocksRef.current, block]); setSlash({ blockId: block.id, query: '', selected: 0 }); focusBlock(block.id) }}><Plus size={16} /> 블록 추가</button><input ref={imageInputRef} className="visually-hidden" type="file" accept="image/*,.heic,.heif" multiple onChange={(event) => void chooseFile(event)} /><input ref={pdfInputRef} className="visually-hidden" type="file" accept="application/pdf,.pdf" multiple onChange={(event) => void chooseFile(event)} /><input ref={fileInputRef} className="visually-hidden" type="file" multiple onChange={(event) => void chooseFile(event)} /><input ref={folderInputRef} className="visually-hidden" type="file" multiple onChange={(event) => void chooseFile(event)} /></div>
}

function EditorList({ blocks, parentId, level, onBlocksChange, onAction, onRemoveBlock, renderMedia, activeId, setActiveId, slash, setSlash, registerInput, focusBlock, chooseCommand, dragging, dropTarget }: { blocks: NoteBlock[]; parentId: string | null; level: number; onBlocksChange: (blocks: NoteBlock[]) => void; onAction: (target: NoteActionTarget) => void; onRemoveBlock: (block: NoteBlock) => void; renderMedia: (block: NoteBlock) => ReactNode; activeId: string | null; setActiveId: Dispatch<SetStateAction<string | null>>; slash: SlashState | null; setSlash: (state: SlashState | null) => void; registerInput: (id: string, node: HTMLTextAreaElement | null) => void; focusBlock: (id: string, caret?: number) => void; chooseCommand: (command: NoteCommand, blocks: NoteBlock[], index: number, commit: (next: NoteBlock[]) => void) => void; dragging: boolean; dropTarget: DropTarget | null }) {
  ...
  return <SortableContext items={blocks.map((block) => block.id)} strategy={verticalListSortingStrategy}><div className={`editor-blocks ${level ? 'editor-blocks--nested' : ''}`}><DropZone parentId={parentId} index={0} dragging={dragging} active={dropTarget?.parentId === parentId && dropTarget.index === 0} />{blocks.map((block, index) => <Fragment key={block.id}><SortableBlock block={block} index={index} number={block.type === 'numbered' ? consecutiveNumber(blocks, index) : undefined} level={level} blocks={blocks} commit={commit} updateAt={updateAt} removeAt={removeAt} onAction={onAction} onRemoveBlock={onRemoveBlock} renderMedia={renderMedia} activeId={activeId} setActiveId={setActiveId} slash={slash} setSlash={setSlash} registerInput={registerInput} focusBlock={focusBlock} chooseCommand={chooseCommand} dragging={dragging} dropTarget={dropTarget} /><DropZone parentId={parentId} index={index + 1} dragging={dragging} active={dropTarget?.parentId === parentId && dropTarget.index === index + 1} /></Fragment>)}</div></SortableContext>
}

function DropZone({ parentId, index, dragging, active }: { parentId: string | null; index: number; dragging: boolean; active: boolean }) {
  const { setNodeRef, isOver } = useDroppable({ id: `drop:${parentId || 'root'}:${index}` })
  return <div ref={setNodeRef} className={`note-drop-zone ${dragging ? 'is-visible' : ''} ${active || isOver ? 'is-active' : ''}`} aria-hidden="true" />
}
