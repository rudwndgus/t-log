...
export function BlockEditor({ block, number, active, inputRef, handleRef, dragHandleProps, customContent, onFocus, onBlur, onChange, onKeyDown, onCheckedChange, onToggle, onAction }: { block: NoteBlock; number?: number; active: boolean; inputRef: RefCallback<HTMLTextAreaElement>; handleRef: RefCallback<HTMLButtonElement>; dragHandleProps: ButtonHTMLAttributes<HTMLButtonElement>; customContent?: ReactNode; onFocus: () => void; onBlur: () => void; onChange: (value: string, textarea: HTMLTextAreaElement) => void; onKeyDown: (event: KeyboardEvent<HTMLTextAreaElement>) => void; onCheckedChange: (checked: boolean) => void; onToggle: () => void; onAction: () => void }) {
  ...
  return <div className={`editable-block block--${block.type} ${active ? 'is-active' : ''} ${block.checked ? 'is-checked' : ''}`}>
    ...
    <div className="block-input-wrap"><textarea ref={setInputRef} rows={1} value={block.content} placeholder={block.type === 'location' ? '장소 이름 입력' : block.type === 'toggle' ? '토글 제목' : block.type === 'embed' ? '임베드할 URL 붙여넣기' : '내용 입력'} onFocus={onFocus} onBlur={onBlur} onChange={(event) => onChange(event.target.value, event.target)} onKeyDown={onKeyDown} />{isLink && !active && <a href={block.content.trim()} target="_blank" rel="noreferrer">링크 열기</a>}{block.type === 'embed' && /^https?:\/\/\S+$/i.test(block.content.trim()) && <div className="embed-preview"><iframe src={block.content.trim()} title="임베드 콘텐츠" loading="lazy" sandbox="allow-scripts allow-same-origin allow-forms allow-popups" /><a href={block.content.trim()} target="_blank" rel="noreferrer">원본에서 열기 · {block.content.trim()}</a></div>}</div>
