...
export interface UploadAttachmentInput { tripId: string; notePageId: string; blockId: string; attachmentId: string; kind: NoteAttachment['kind']; file: File; createdBy: string; signal?: AbortSignal; onProgress?: (progress: number) => void }
...
const decodeImage = async (blob: Blob) => {
  ...
  return await new Promise<HTMLImageElement>((resolve, reject) => { const url = URL.createObjectURL(blob); const image = new Image(); image.onload = () => { resolve(image); URL.revokeObjectURL(url) }; image.onerror = () => { reject(new Error('UNSUPPORTED_IMAGE')); URL.revokeObjectURL(url) }; image.src = url })
}
...
export async function prepareImageAttachment(file: File): Promise<PreparedAttachment> {
  ...
  const sourceWidth = source instanceof ImageBitmap ? source.width : source.naturalWidth; const sourceHeight = source instanceof ImageBitmap ? source.height : source.naturalHeight
  ...
  context.drawImage(source, 0, 0, width, height); if (source instanceof ImageBitmap) source.close()
