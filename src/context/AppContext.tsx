...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const upsertSegment = async (segment: TransportSegment) => { const original = data.segments.find((item) => item.id === segment.id); mutate((current) => ({ ...current, segments: [...current.segments.filter((item) => item.id !== segment.id), segment] })); try { if (db && signedIn) await saveSegmentDocument(db, segment) } catch (error) { mutate((current) => ({ ...current, segments: [...current.segments.filter((item) => item.id !== segment.id), ...(original ? [original] : [])] })); reportCloudError('save-segment', error, { tripId: segment.tripId, userId: profile.id }); throw error } }
