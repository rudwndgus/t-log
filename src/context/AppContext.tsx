...
interface AppContextValue {
  ...
  createTrip: (input: CreateTripInput) => Promise<Trip>; deleteTrip: (tripId: string) => Promise<void>; shareTrip: (tripId: string) => Promise<string>; joinTrip: (code: string) => Trip | null; joinCloudTrip: (code: string) => Promise<string>
  addNotePage: (tripId: string, title?: string) => Promise<NotePage>; updateNotePage: (pageId: string, changes: Partial<Pick<NotePage, 'title' | 'blocks'>>) => void; deleteNotePage: (pageId: string) => Promise<void>; duplicateNotePage: (pageId: string) => Promise<NotePage | undefined>; reorderNotePages: (tripId: string, oldIndex: number, newIndex: number) => void; addBlock: (pageId: string, block: Omit<NoteBlock, 'id'>) => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const shareTrip = async (tripId: string) => { const trip = data.trips.find((item) => item.id === tripId); if (!trip) throw new Error('TRIP_NOT_FOUND'); if (!db || !signedIn) throw new Error('CLOUD_REQUIRED'); const shareId = trip.publicShareId || uid().replaceAll('-', '').slice(0, 20); try { await publishTripDocument(db, trip, data.places, shareId, profile.id); mutate((current) => ({ ...current, trips: current.trips.map((item) => item.id === tripId ? { ...item, publicShareId: shareId } : item) })); return `${location.origin}${location.pathname}#/shared/${shareId}` } catch (error) { reportCloudError('publish-trip', error, { tripId, userId: profile.id }); throw error } }
  const joinTrip = (code: string) => data.trips.find((trip) => trip.inviteCode === code.trim().toUpperCase()) || null
  const joinCloudTrip = async (code: string) => { if (!db || !signedIn) throw new Error('AUTH_REQUIRED'); try { return await joinTripByCode(db, code, profile) } catch (error) { reportCloudError('join-trip', error, { inviteCode: code.toUpperCase(), userId: profile.id }); throw error } }
